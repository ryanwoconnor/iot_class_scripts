...
class AtlasI2C:
        ...
        default_address = 104           # the default address for the sensor
        ...
        def read(self, num_of_bytes=31):
                ...
                if res[0] == 1:
                        ...
                        jsonDict = {'host': str(socket.gethostname()), 'event': 'metric', 'index': index, 'fields':{'Flow_Total':str(values[0]),'_value':str(values[0]),'metric_name':'Flow_Total'}}
                        r = requests.post(url,headers=authHeader,json=jsonDict,verify=False)
                        jsonDict = {'host': str(socket.gethostname()), 'event': 'metric', 'index': index, 'fields':{'Flow_Rate':str(values[1]),'_value':str(values[1]),'metric_name':'Flow_Rate'}}
                        r = requests.post(url,headers=authHeader,json=jsonDict,verify=False)
                        #print(jsonDict)
                        #print(r.text)
...
def main():
        ...
        #Crontab section. Runs 1 Time and is done
